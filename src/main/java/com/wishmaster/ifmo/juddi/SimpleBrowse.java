...
public class SimpleBrowse {
        ...
        public void FindServiceByName(String name){
                try {
                        String token = GetAuthKey("uddiadmin", "da_password1");
                        ServiceList services = GetServiceListByName(token, name);
                        PrintServiceInfo(token,services.getServiceInfos());
        ...
        public void PrintAllServices(){
                try {
                        String token = GetAuthKey("uddiadmin", "da_password1");
                        ServiceList services = GetServiceList(token);
                        PrintServiceInfo(token,services.getServiceInfos());
                        security.discardAuthToken(new DiscardAuthToken(token));
        ...
        private ServiceList GetServiceListByName(String token, String name) throws RemoteException {
        ...
        private ServiceList GetServiceList(String token) throws RemoteException {
                FindService fs = new FindService();
                fs.setAuthInfo(token);
                org.uddi.api_v3.FindQualifiers fq = new org.uddi.api_v3.FindQualifiers();
                fq.getFindQualifier().add(UDDIConstants.APPROXIMATE_MATCH);
...
//                                PrintServiceInfo(businessInfos.getBusinessInfo().get(i).getServiceInfos());
        ...
        private void PrintServiceInfo(String token, ServiceInfos serviceInfos) throws RemoteException {
                for (int i = 0; i < serviceInfos.getServiceInfo().size(); i++) {
                        ...
                        gsd.getServiceKey().add(serviceInfos.getServiceInfo().get(i).getServiceKey());
                        gsd.setAuthInfo(token);
                        System.out.println("Fetching data for service " + serviceInfos.getServiceInfo().get(i).getServiceKey());
